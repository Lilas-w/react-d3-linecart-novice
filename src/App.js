      ...
      useEffect(
        ...
        //  3] Setup functions for Scales ------------------//
            ...
            const xScale = d3.scalePoint()
                            .domain(chartdata.map( (d) => d.name ))  //所有的点
                            .range([(0+padding),(width - padding)])  //画布范围
            console.log('Start - End',xScale('Car'),xScale('Cinema'))                

            //Yscales
            const yScale = d3.scaleLinear()
                             .domain([0, d3.max( chartdata, function (d) {return d.value})]) // max : used with accessors that retrieve a specific property from the given objects
        ...
        //  4] Setup functions to draw Lines ---------------//
            ...
            const line = d3.line()
                           .x((d)=> xScale(d.name)) // 传入x轴上的name名
                           .y((d)=> yScale(d.value)) // 传入y轴上的value值
                           .curve(d3.curveMonotoneX) // 假定数据根据 x 坐标排序
  ...
  return (
    <div className="App">
      <header className="App-header">
        ...
        <p>
          <Button type='button' variant="contained" onClick={()=> setChartdata(newData())}>
